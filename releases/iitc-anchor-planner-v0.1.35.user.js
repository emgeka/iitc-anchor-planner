...
function wrapper(plugin_info) {
  ...
  plugin_info.dateTimeVersion = '20260713173000';
  ...
  ap.collectExistingLinkIds = function () {
    var map = {};
    var list = [];
    var count = 0;
    var unresolved = 0;
    ...
    Object.keys(window.links).forEach(function (guid) {
      ...
      var info = { guid: guid, a: pair.a, b: pair.b, source: pair.source, latlngA: coords && coords.a, latlngB: coords && coords.b };
      if (!map[id]) count++;
      map[id] = info;
      list.push(info);
    });
  ...
  ap.findBlockersForPlannedLink = function (planned, existingLinks) {
    ...
    (existingLinks || []).forEach(function (real) {
      ...
      if (ap.properSegmentsIntersect(planned.latlngA, planned.latlngB, real.latlngA, real.latlngB)) {
        blockers.push({ guid: real.guid, a: real.a, b: real.b });
      }
    });
    return blockers;
  };
  ...
  ap.getNextPortal = function () {
    var stats = ap.sortedStats(false);
    for (var i = 0; i < stats.length; i++) {
      if (!ap.ensureAnchorState(stats[i].guid).done) return stats[i];
    }
    return null;
  };
  ...
  ap.scan = function () {
    ...
    var existingInfo = ap.collectExistingLinkIds();
  ...
  ap.buildPlanText = function () {
    ...
    lines.push('Vorhandene Links: ' + (last.existingPlannedLinks || 0) + ' · nicht bestätigt: ' + (last.openPlannedLinks || 0));
  ...
  ap.renderPanel = function () {
    ...
    html += '<div class="ap-actions"><button id="ap-scan">Draw Tools scannen</button><button id="ap-loadnames">Namen laden</button><button id="ap-export">Export / Teilen</button><button id="ap-clear">Daten löschen</button></div>';
    ...
    if (nextPortal) {
      ...
      html += '<div class="ap-next-target"><span><b>Nächstes Portal:</b> ' + ap.escapeHtml(nextPortal.title) + '</span><a target="_blank" rel="noopener" href="' + ap.escapeHtml(nextNav.waze) + '">Waze</a></div>';
    } else if (stats.length) {
      html += '<div class="ap-next-target ap-next-complete">Alle Planportale erledigt.</div>';
    }
