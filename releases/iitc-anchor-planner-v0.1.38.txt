...
function wrapper(plugin_info) {
  ...
  plugin_info.dateTimeVersion = '20260714124404';
    ...
    if (blockedLinks.length) {
      ...
      blockedLinks.forEach(function (link) {
        ...
        link.blockers.forEach(function (blocker) {
          ...
          html += '<div class="ap-blocker-existing">Blockiert durch: ' + ap.escapeHtml(blockerA) + ' ↔ ' + ap.escapeHtml(blockerB) + '</div>';
        });
        html += '</div>';
      });
